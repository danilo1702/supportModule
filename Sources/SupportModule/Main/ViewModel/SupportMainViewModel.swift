        ...
        reference.addSnapshotListener { [weak self] querySnapshot, error in
            ...
            querySnapshot.documentChanges.forEach { change in
                ...
                if let message = try? change.document.data(as: MessageModel.self) {
                    ...
                    self.recentMessage.insert(CardModel(id: message.id ?? "1", titleFormat: TextViewModel(text: message.message, foregroundColor: .black, font: .system(size: 14), expandable: false), designCard: ComponentDesign(backgroundColor: .gray.opacity(0.1), cornerRaiuds: 15),toUUID: message.toUUID, action: "chat"), at: 0)
                }
            }
        }
    }
